    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        input:focus{outline:none;border-color:rgba(123,94,168,0.6)!important;}
        @keyframes fadeUp{from{opacity:0;transform:translateY(16px);}to{opacity:1;transform:translateY(0);}}
        .fu{animation:fadeUp 0.7s ease forwards;}
      `}</style>

      <div style={{ minHeight:'100vh', background:clr.bg, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', fontFamily:"'DM Sans', sans-serif", padding:'24px' }}>

        {/* Logo */}
        <a href="/" style={{ fontFamily:"'Cormorant Garamond', serif", fontSize:24, letterSpacing:'0.25em', color:clr.purpleLight, textDecoration:'none', marginBottom:48 }}>
          PHOEBIX
        </a>

        {/* Card */}
        <div className="fu" style={{ width:'100%', maxWidth:400, background:clr.nebula, border:`0.5px solid ${clr.border}`, borderRadius:20, padding:'36px 32px' }}>
            ...
            {mode==='register' && (
              <div>
                ...
                <input value={name} onChange={e => setName(e.target.value)}
                  placeholder="Adınız"
                  style={{ width:'100%', background:'rgba(4,2,14,0.8)', border:`0.5px solid ${clr.border}`, borderRadius:10, padding:'11px 14px', fontSize:14, color:clr.star, fontFamily:"'DM Sans', sans-serif" }} />
              </div>
            )}
            <div>
          ...
          <button onClick={handleSubmit} disabled={loading} style={{
            width:'100%', marginTop:24, padding:'13px 0', background: loading ? 'rgba(123,94,168,0.4)' : clr.purple,
